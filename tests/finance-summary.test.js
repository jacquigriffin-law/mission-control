...
test("data/finance-summary.json fallback supplies profitLoss when env is absent", () => {
  ...
  assert.ok(summary.actions.some((action) => action.id === "carried-gst-pressure"));
  assert.ok(summary.actions.some((action) => action.id === "unallocated-items"));
  const actionKeys = [
    "amount",
    "detail",
    "dueDate",
...
test("Finance screen binds only to safe aggregate finance fields", () => {
  ...
  assert.ok(indexHtml.includes("renderFinanceActions(data.actions)"), "finance screen must render data.actions");
  assert.ok(indexHtml.includes("item.source"), "finance action renderer must show aggregate source");
  [
    /rawMyob/,
    /rawRecords/,
    /clientRecords/,
